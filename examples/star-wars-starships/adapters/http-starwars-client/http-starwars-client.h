...
public:
    HttpStarWarsClient(
        const std::string& url,                                 //where to reach find the star wars endoints? (e.g. https://swapi.dev)
        const std::vector<std::string>& headers_to_propagate    //which headers shall be propagated from this service to the star wars service? (e.g. tracing headers or authorization headers)        
    );
