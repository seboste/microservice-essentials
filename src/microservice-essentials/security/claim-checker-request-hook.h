...
public:    
    ...
    struct Parameters
    {
        ...
        AutoRequestHookParameterRegistration<ClaimCheckerRequestHook::Parameters, ClaimCheckerRequestHook> auto_registration;
    };

    static mse::Status PermissionDenied;
    static Parameters ClaimEqualTo(const std::string& claim, const std::string& value, mse::Status fail_status = PermissionDenied);
    static Parameters ClaimInSpaceSeparatedList(const std::string& claim, const std::string& value, mse::Status fail_status = PermissionDenied);
    static Parameters ScopeContains(const std::string& value) { return ClaimInSpaceSeparatedList("scope", value); }
